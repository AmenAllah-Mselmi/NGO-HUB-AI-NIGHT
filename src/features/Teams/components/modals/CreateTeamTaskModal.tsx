...
import { Plus, X, Check, Paperclip, Loader2, Flag, Trophy } from "lucide-react";
...
interface CreateTeamTaskModalProps {
    ...
    onClose: () => void;
    teamId: string;
    teamMembers: { id: string; fullname: string; avatar_url?: string }[];
    onCreated: () => void;
...
export default function CreateTeamTaskModal({
    ...
    const [complexity, setComplexity] = useState<'lead' | 'major' | 'minor' | ''>('');
    ...
    useEffect(() => {
        ...
        // Fetch milestones
        getTeamMilestones(teamId).then(data => {
            setMilestones(data);
        }).catch(err => console.error("Could not fetch milestones", err));

    }, [open, initialStatus, teamId]);
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            const newTask = await createTask({
                ...
                complexity: (complexity || undefined) as 'lead' | 'major' | 'minor' | undefined,
                team_id: teamId,
            ...
            <div className="relative w-full sm:max-w-5xl bg-white h-full sm:h-auto sm:max-h-[90vh] rounded-none sm:rounded-[2.5rem] shadow-2xl z-10 overflow-hidden flex flex-col animate-in slide-in-from-bottom sm:zoom-in-95 duration-300">
                ...
                <div className="p-4 sm:p-6 border-b flex justify-between items-center bg-white shrink-0">
                    <div className="flex items-center gap-3 sm:gap-4">
                        ...
                        >
                            <input type="color" className="absolute inset-0 opacity-0 cursor-pointer scale-150" value={headerColor || '#f3f4f6'} onChange={(e) => setHeaderColor(e.target.value)} />
                            {!headerColor && <Plus className="w-4 h-4 sm:w-5 sm:h-5 text-gray-400" />}
                        </div>
                        <div>
                ...
                <div className="flex-1 flex flex-col md:flex-row overflow-y-auto md:overflow-hidden">
                    ...
                    <div className="p-4 sm:p-6 space-y-4 sm:space-y-6 md:flex-1 md:overflow-y-auto md:border-r border-gray-100">
                        ...
                        <section className="space-y-4">
                            <div className="flex flex-wrap gap-2">
                                {THEME_COLORS.map(c => (
                                    <button key={c.hex} onClick={() => setHeaderColor(c.hex)} className={`w-6 h-6 rounded-full border-2 transition-all ${headerColor === c.hex ? 'border-gray-900 scale-110 shadow-sm' : 'border-white shadow-inner'}`} style={{ backgroundColor: c.hex }} title={c.name} />
                                ))}
                            </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="block text-[10px] font-black uppercase tracking-widest text-gray-500 mb-1 flex items-center gap-1"><Trophy className="w-3 h-3 text-amber-500" /> Points *</label>
                                <input type="number" min="0" required value={points} onChange={(e) => setPoints(e.target.value)} className="w-full bg-white border border-gray-200 rounded-xl px-4 py-2 text-sm focus:border-blue-500 outline-none transition-colors" placeholder="e.g. 10" />
                            </div>
                            <div>
                                <label className="block text-[10px] font-black uppercase tracking-widest text-gray-500 mb-1 flex items-center gap-1"><Flag className="w-3 h-3" /> Complexity</label>
                                <select value={complexity} onChange={e => setComplexity(e.target.value as any)} className="w-full bg-white border border-gray-200 rounded-xl px-4 py-2 text-sm focus:border-blue-500 outline-none transition-colors">
                        ...
                        {milestones.length > 0 && (
                            <div>
                                <label className="block text-[10px] font-black uppercase tracking-widest text-gray-500 mb-1 flex items-center gap-1 text-emerald-600">
                                    <Flag className="w-3 h-3 text-emerald-500" /> Link to Milestone
                                </label>
                                <select value={milestoneId} onChange={e => setMilestoneId(e.target.value)} className="w-full bg-white border border-gray-200 rounded-xl px-4 py-2 text-sm focus:border-emerald-500 outline-none transition-colors">
                            ...
                            <div className="flex gap-2">
                                <select className="flex-1 px-2 py-1.5 bg-white border border-gray-200 rounded-xl text-[10px] sm:text-xs font-bold focus:ring-2 focus:ring-blue-500 outline-none" value={status} onChange={e => setStatus(e.target.value as any)}>
                                    <option value="todo">To Do</option>
                                    <option value="in_progress">In Progress</option>
                                    <option value="completed">Completed</option>
                                </select>
                                <input type="date" className="w-28 sm:w-32 px-2 py-1.5 bg-white border border-gray-200 rounded-xl text-[9px] sm:text-[10px] font-bold outline-none" value={deadline} onChange={e => setDeadline(e.target.value)} />
                            </div>
                        </div>
                    </div>
                    ...
                    {status === 'completed' && (
                        <div className="p-4 bg-amber-50 rounded-2xl border border-amber-100 flex items-center lg:flex-row flex-col gap-4">
                            <StarRating value={starRating} onChange={val => setStarRating(val)} />
