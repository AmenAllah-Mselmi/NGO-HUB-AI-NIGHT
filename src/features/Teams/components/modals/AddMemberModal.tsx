...
export default function AddMemberModal({ open, onClose, teamId, existingMemberIds, onAdded }: AddMemberModalProps) {
    ...
    const handleAdd = async () => {
        ...
        try {
            // Add all selected
            await Promise.all(selectedIds.map(mid => addTeamMember(teamId, mid, 'member')));
            
            toast.success(`Added ${selectedIds.length} members to the team`);
            onAdded();
            onClose();
        } catch (error) {
                ...
                <div className="flex-1 overflow-y-auto p-2">
                    {loading ? (
                        ...
                        <div className="p-8 text-center text-gray-400">
                             {members.length === 0 ? "All available members are already in this team." : "No members found matching search."}
                        </div>
                    ) : (
